...
class SetupCommand extends Command
{
    ...
    {
        $this->setName('fpm:setup')
            ->setDescription('Generate and setup FPM startup config')
            ->setHelp('Usage: phpbrew fpm:setup [--systemctl|--initd|--launchctl] [--stdout] [<build-name>]')
            ->addOption(
                'systemctl',
                null,
            ...
            ->addOption(
                ...
                null,
                InputOption::VALUE_NONE,
                'Generate plist for launchctl (OS X)'
            )
            ->addOption(
                'stdout',
                null,
                InputOption::VALUE_NONE,
                'Print config to STDOUT instead of writing to the file.'
            )
            ->addArgument(
                'build-name',
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $buildName = $input->getArgument('build-name');
        if (!$buildName) {
        ...
        if ($input->getOption('systemctl')) {
            ...
            if (!is_writable(dirname($file))) {
                $io->error("$file is not writable.");
                return Command::FAILURE;
            }

            $io->writeln("Writing systemctl service entry: $file");
            file_put_contents($file, $content);

            $io->writeln('To reload systemctl service:');
            $io->writeln('    systemctl daemon-reload');
            $io->writeln("Ensure that $buildName was built with --fpm-systemd option");
        } elseif ($input->getOption('initd')) {
            ...
            $io->writeln("Writing init.d script: $file");
            file_put_contents($file, $content);
            chmod($file, 0755);

            $io->writeln('To setup the startup item, remember to run update-rc.d to link the init script:');
            $io->writeln('    sudo update-rc.d phpbrew-fpm defaults');
        } elseif ($input->getOption('launchctl')) {
            ...
            $io->writeln("Writing launchctl plist file: $file");
            file_put_contents($file, $content);

            $io->writeln('To load the service:');
            $io->writeln("    sudo launchctl load $file");
        } else {
            $io->writeln(
    ...
    protected function generateLaunchctlService(string $buildName, string $fpmBin): string
    {
        $root   = Config::getRoot();
...
EOS;
    ...
    protected function generateSystemctlService(string $buildName, string $fpmBin): string
    {
        $root    = Config::getRoot();
        $phpdir  = "$root/php/$buildName";
...
EOS;
    ...
    protected function generateInitD(string $buildName, string $fpmBin): string
    {
        $root    = Config::getRoot();
        $phpdir  = "$root/php/$buildName";
        $pidFile = $phpdir . '/var/run/php-fpm.pid';
